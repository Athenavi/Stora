            ...
            {items.value.map(f => (
              <div key={f.id}
                ...
                <button onClick$={(e: any) => { e.stopPropagation(); removeFav(f.id); }}
                  class="absolute top-1.5 right-1.5 w-8 h-8 rounded-full bg-white/90 text-red-400 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-sm hover:bg-red-50 shadow-sm touch-target">✕</button>
              </div>
            ))}
          </div>
        )}
