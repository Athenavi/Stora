  ...
  return (
    <div class="flex flex-col h-full">
      <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white">
        ...
        <span class="text-sm text-slate-400 bg-slate-100 px-3 py-1 rounded-full">{shares.value.length} 个链接</span>
      </div>
      ...
      <div class="flex-1 overflow-auto scrollbar-thin p-6">
        {shares.value.length === 0 ? (
          ...
          <div class="space-y-3">
            {shares.value.map((s) => (
              <div key={s.id} class="flex items-center gap-4 px-5 py-4 bg-white rounded-xl border border-slate-200 hover:shadow-sm transition-shadow">
